...
resource "aws_api_gateway_rest_api" "devops_api" {
  name        = "devops-api-level2"
  description = "DevOps Assignment Level 2 - Terraform API Gateway"
...
resource "aws_api_gateway_resource" "json_proxy" {
  rest_api_id = aws_api_gateway_rest_api.devops_api.id
...
resource "aws_api_gateway_method" "json_get" {
  rest_api_id   = aws_api_gateway_rest_api.devops_api.id
  resource_id   = aws_api_gateway_resource.json_proxy.id
  http_method   = "GET"
  authorization = "NONE"
